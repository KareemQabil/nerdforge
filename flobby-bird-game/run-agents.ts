...
async function run() {
  ...
  const pending = microtasks.filter((m: any) => state.pendingMicrotasks.includes(m.id));
  ...
  for (const mt of pending) {
    ...
    const confirmApprove = async () => 'commit' as const;
    ...
    if (result.success) {
      ...
      stateManager.update({
        pendingMicrotasks: currentState.pendingMicrotasks.filter((id: string) => id !== mt.id),
      ...
      if (result.gatekeeperVerdict) {
        ...
        for (const reason of result.gatekeeperVerdict.reasons) console.error(` - ${reason}`);
      }
      break;
    }
  }
